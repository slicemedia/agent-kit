...
6. Test the real rendered markup: Webflow readiness, matching `data-wft-*` hooks, multiple/CMS instances, delayed content, breakpoints, reduced motion, errors, refresh, destroy, and reinitialize. Confirm each HMR update replaces behavior rather than stacking listeners or instances. Suggest the optional on-page DevTools inspector when loaded; on localhost, enable it with `window.DevKitDevTools.enabled = true` before calling `window.DevKitDevTools.open()`. Use Rescan after HMR, CMS, or viewport changes to compare requirements, effective options, and runtime reports. Missing diagnostics remain unverified, and a clean scan does not replace interaction tests.
