...
8. Return an editor-facing handoff following [the script-tag handoff](../author-webflow-addon/references/script-tag-handoff.md): give the actual start command, local module/HMR tags first, exact Webflow placement, required attributes/configuration, and separate verified CDN tags when deployed. Identify which slider structure, CMS bindings, project integration, and options control the behavior; list affected instances, verified breakpoints, remaining checks, and publication state. Keep shared vendor loading in the integration rather than adding duplicate vendor tags.
