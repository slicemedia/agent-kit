...
Name the owning instance, component, CMS template/collection, page, or site and the exact control surface: Webflow style, native Interaction, or local GSAP/CSS module. For addons, follow [the script-tag handoff](../author-webflow-addon/references/script-tag-handoff.md): give the start command, local module/HMR tags first, exact Webflow placement, and separate verified CDN script/CSS tags when deployed, along with required plugins, markup hooks, options, and public lifecycle methods/events. Explain where future edits belong, which reused instances, breakpoints, properties, and scripts are also affected, and the verification, publication, restore-point, or waiver state. Native Interaction or CSS-only work needs its own setup instructions rather than invented addon tags.
