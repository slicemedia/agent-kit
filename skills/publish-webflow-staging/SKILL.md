...
6. If rendered browser and network evidence is available, verify the staging URL, loaded addon and optional project script versions, network requests, console, accessibility-critical path, and requested behavior. Otherwise report the publication request as accepted but rendered verification as unavailable; never claim success from MCP acceptance alone. Do not auto-republish after failure.
