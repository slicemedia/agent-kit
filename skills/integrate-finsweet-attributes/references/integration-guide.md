...
Before every Webflow-hosted mutation, present the exact bounded operations, targets, shared consumers, preserved state, and recovery limitations. Ask the user to save all current changes and create a new native Webflow restore point, or to explicitly waive it after those limits are explained, then hard-stop for a new reply. Old or automatic backups, activity history, DOM snapshots, screenshots, exported inventories, post-write read-back, and advance approvals do not count. Only after the reply, re-read affected state and ask for a separate final confirmation immediately before the first write. The recovery reply cannot double as write confirmation. Restart the gate if state or scope changed; one gate covers only its unchanged bounded batch.
