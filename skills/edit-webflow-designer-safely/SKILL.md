...
5. Require explicit confirmation of the exact site, scope, current plan, and expected blast radius before any write. A prior inspection or general request to improve the site is not write confirmation.
6. Use the focused tool: `data_element_tool`, `data_element_settings_tool`, `data_component_tool`, `data_component_props_tool`, `data_component_variants_tool`, `data_style_tool`, or `data_variable_tool`.
7. Read back the same scope and compare with the plan. Stop automatic continuation after a partial failure or stale result. Use `element_snapshot_tool` only when visual verification is needed and the bridge is available.
8. Restore the original Designer page/component view after temporary navigation. Return a receipt containing the risk tier, recovery checkpoint, changes, preserved state, mismatches, partial failures, errors, and `published: false`.
9. Provide an editor-facing handoff naming the affected scope and whether future edits affect one element, one component instance, every component instance, one page, shared styles or variables, CMS content, or the entire site.
