...
8. Return an editor-facing handoff following [the script-tag handoff](references/script-tag-handoff.md): present the start command, local module/HMR tags first, exact Webflow placement and scope, required attributes/configuration, and separate verified CDN tags when deployed. Include the source entry, public API/events, affected instances/pages, tests completed, unverified cases, and `published: false`. A browser test or source link does not replace copyable user-facing snippets.
