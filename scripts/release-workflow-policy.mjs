...
const expectedWorkflow = {
  ...
  permissions: {},
  concurrency: {
    group: "agent-kit-release-pr",
    "cancel-in-progress": false,
  },
  jobs: {
    version: {
      if: "github.repository == 'slicemedia/agent-kit' && vars.SLICEMEDIA_RELEASE_PR_ENABLED == 'true'",
      ...
      steps: [
        ...
        },
        {
          ...
          with: {
            ...
            "commit-message": "chore: version Agent Kit",
            "pr-title": "chore: version Agent Kit",
            "pr-draft": "create",
          },
        },
      ],
...
const allowedActionReferences = [
  checkoutAction,
  pnpmSetupAction,
  changesetsVersionAction,
];
const allowedRunCommands = [
  "pnpm install --frozen-lockfile",
  "pnpm release:prepare:check",
  "pnpm sanitize",
];
const prohibitedWorkflowPatterns = [
  ...
  ["npm token path", /\b(?:NODE_AUTH_TOKEN|NPM_TOKEN)\b/u],
...
export function validateReleaseWorkflow(source) {
  ...
  for (const [label, expression] of prohibitedWorkflowPatterns) {
    if (expression.test(source))
      errors.push(`Version-PR workflow contains a prohibited ${label}.`);
  }
  ...
  for (const warning of document.warnings) {
    errors.push(
      `Version-PR workflow YAML warning is not allowed: ${warning.message}`,
    );
  }
  ...
  const runCommands = [...source.matchAll(/^\s*-\s+run:\s*(.+)$/gmu)].map(
    (match) => match[1].trim(),
  );
