...
- Use neutral `data-wft-*` hooks. Give each new addon with markup hooks a stable `data-wft-<addon>` root and `data-wft-<addon>-<role-or-option>` names. Check existing contracts for collisions; different addons need distinct prefixes, while repeated instances reuse the same names with root-scoped ownership, including nested instances. Document deliberate shared hooks and preserve existing contracts unless migration is requested. Never introduce client names, assets, URLs, site IDs, component IDs, credentials, copied production markup, or project fallback selectors.
